...
/*
In the following modified code, we load 16 elements at a time into NEON registers
using vld1q_s8 and subtract them using vsubq_s8. We then calculate the absolute
difference using vabsq_s8. The resulting absolute differences are accumulated
into the sad vector using vaddq_s8. Finally, we reduce the 8-bit accumulations
to a single 64-bit value using pairwise additions with vpaddlq_s8, vpaddlq_s16,
vpaddlq_s32, and vgetq_lane_s64.
*/

// CONSTRAINT: BLOCK_SIZE is 16
int sad(int8_t A[BLOCK_SIZE][BLOCK_SIZE], int8_t B[BLOCK_SIZE][BLOCK_SIZE])
{
    ...
    We interleaved the instructions in a way that avoids consecutive
    dependent instructions, allowing for parallel execution.
    */
    for (i = 0; i < BLOCK_SIZE; i+=2)
    {
        /* 
        These are contiguous memory accesses.
        This is efficient for the cache, prefetching, and data transfer
        */
...
To improve the init_block function, we take advantage of Neon's 128-bit registers
by loading and storing multiple elements at once.

Explanation:

We use int8x16_t to represent an 16-element vector of 8-bit integers, which corresponds
to a Neon register. The inner loop is executed BLOCK_SIZE times to load and store elements
column-wise. The vld1q_s8 function loads a Neon register with 16 elements from the image
array starting at the specified address (&image[x + i][y + 0]). The vst1q_s8 function stores
the 16 elements from the Neon register to the block array starting at the specified address (&block[i][0]).

Using Neon intrinsics in init_block enables loading and storing multiple elements at once,
improving the memory access efficiency and performance.
*/
...
void init_block(int x, int y, int8_t image[IMG_W][IMG_H], int8_t block[BLOCK_SIZE][BLOCK_SIZE])
{
    ...
    {
        // The inner loop is removed and j is replaced with 0 because we load a whole row at a time
...
/*
This function initializes an image with a rectangle
of size shape_w, shape_h at starting point shape_x, shape_y.
*/
